# Unguarded FO Same-Radius Disjunction Constructor Lock

PROGRESS_ID := cslib_fmt_disjunction_constructor_lock_2026_06_20

STATUS := UNGUARDED_FO_DISJ_SAME_RADIUS_CONSTRUCTOR_LOCK_ONLY

This status lock records the same-radius disjunction constructor in the
...
`Formula.disj φ ψ` at the same radius `r`.
...
The weakest next admissible formal step is a shared-radius Boolean rollup over
the three one-step constructors already present: negation, conjunction, and
disjunction.

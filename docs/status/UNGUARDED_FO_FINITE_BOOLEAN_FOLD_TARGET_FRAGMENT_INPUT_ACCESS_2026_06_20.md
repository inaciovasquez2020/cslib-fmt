# Unguarded FO finite Boolean fold target-fragment input access lemmas — 2026-06-20

STATUS := `FINITE_BOOLEAN_FAMILY_FOLD_TARGET_FRAGMENT_INPUT_ACCESS_LEMMAS`
...
This layer extracts expression-index scoped fold target-fragment input access lemmas for atom, negation, conjunction, and disjunction cases.

The theorem statements remain over `FiniteBooleanFamilyExpr`.

BOUNDARY :=

This is expression-index scoped target-fragment input access for the explicit finite Boolean fold only, not arbitrary bounded-fragment closure.
...
NEXT_TARGET := extract expression-index scoped fold target-locality input access lemmas without generalizing to arbitrary fragments.

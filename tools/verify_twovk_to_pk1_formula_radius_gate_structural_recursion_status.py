...
ART = Path("artifacts/twovk_to_pk1_formula_radius_gate_structural_recursion_status_2026_06_21.json")
DOC = Path("docs/status/TWOVK_TO_PK1_FORMULA_RADIUS_GATE_STRUCTURAL_RECURSION_STATUS.md")
...
if not re.search(r"exact hStatus\.(?:1|2)\.(?:1|2)", src):
    raise SystemExit("MISSING_OBJECT := direct nested .1/.2 projection from hStatus")

if artifact.get("status") != "TWOVK_TO_PK1_FORMULA_RADIUS_GATE_STRUCTURAL_RECURSION_STATUS_ONLY":
    raise SystemExit("MISSING_OBJECT := artifact status lock")

if artifact.get("target_lean_definition") != "formula_structural_recursion_assembler_target":
...
print("TWOVK_TO_PK1_FORMULA_RADIUS_GATE_STRUCTURAL_RECURSION_STATUS_OK")

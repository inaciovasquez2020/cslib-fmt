...
class LibraryBoundaryCertificate:
    theorem_id: str
    status: str
    required_count: int
    present_count: int
...
def library_boundary_certificate(root: Path | str, required: Sequence[str], boundary_text: str) -> LibraryBoundaryCertificate:
    normalized = _normalize_required(required)
    ...
    return LibraryBoundaryCertificate(
        theorem_id="CSLIB-LBC-1",

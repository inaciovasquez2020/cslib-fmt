...
artifact_path = Path("artifacts/cslib_fmt/unguarded_fo_finite_boolean_fold_membership_2026_06_20.json")
doc_path = Path("docs/status/UNGUARDED_FO_FINITE_BOOLEAN_FOLD_MEMBERSHIP_2026_06_20.md")
...
required_scope = [
    ...
    "expression-index scoped fold-output membership only",
...
assert artifact["status"] == "FINITE_BOOLEAN_FAMILY_FOLD_FRAGMENT_MEMBERSHIP_LEMMAS"
assert "NEXT_TARGET := extract expression-index scoped fold radius/input access lemmas" in doc

print("UNGUARDED_FO_FINITE_BOOLEAN_FOLD_MEMBERSHIP_OK")

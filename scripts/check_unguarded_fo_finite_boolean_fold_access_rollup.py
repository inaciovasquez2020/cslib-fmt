...
artifact_path = Path("artifacts/cslib_fmt/unguarded_fo_finite_boolean_fold_access_rollup_2026_06_20.json")
doc_path = Path("docs/status/UNGUARDED_FO_FINITE_BOOLEAN_FOLD_ACCESS_ROLLUP_2026_06_20.md")
...
for name in required_uses:
    ...
    assert name in artifact["uses_existing_objects"], name
    assert name in doc, name

required_scope = [
    "theorem statement remains over FiniteBooleanFamilyExpr",
    "compact expression-index scoped finite Boolean fold access rollup only",
...
assert artifact["status"] == "FINITE_BOOLEAN_FAMILY_FOLD_ACCESS_ROLLUP_THEOREM"
assert "NEXT_TARGET := add a final finite Boolean fold access status lock" in doc

print("UNGUARDED_FO_FINITE_BOOLEAN_FOLD_ACCESS_ROLLUP_OK")

...
artifact_path = Path("artifacts/cslib_fmt/unguarded_fo_finite_boolean_fold_shared_radius_input_access_2026_06_20.json")
doc_path = Path("docs/status/UNGUARDED_FO_FINITE_BOOLEAN_FOLD_SHARED_RADIUS_INPUT_ACCESS_2026_06_20.md")
...
required_scope = [
    ...
    "expression-index scoped atom shared-radius input access only",
    "expression-index scoped neg shared-radius input access only",
    "expression-index scoped conj shared-radius input access only",
    "expression-index scoped disj shared-radius input access only",
...
assert artifact["status"] == "FINITE_BOOLEAN_FAMILY_FOLD_SHARED_RADIUS_INPUT_ACCESS_LEMMAS"
assert "NEXT_TARGET := extract expression-index scoped fold target-fragment input access lemmas" in doc

print("UNGUARDED_FO_FINITE_BOOLEAN_FOLD_SHARED_RADIUS_INPUT_ACCESS_OK")

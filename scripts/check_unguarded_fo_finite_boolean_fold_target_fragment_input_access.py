...
artifact_path = Path("artifacts/cslib_fmt/unguarded_fo_finite_boolean_fold_target_fragment_input_access_2026_06_20.json")
doc_path = Path("docs/status/UNGUARDED_FO_FINITE_BOOLEAN_FOLD_TARGET_FRAGMENT_INPUT_ACCESS_2026_06_20.md")
...
required_scope = [
    ...
    "expression-index scoped atom target-fragment input access only",
    "expression-index scoped neg target-fragment input access only",
    "expression-index scoped conj target-fragment input access only",
    "expression-index scoped disj target-fragment input access only",
...
assert artifact["status"] == "FINITE_BOOLEAN_FAMILY_FOLD_TARGET_FRAGMENT_INPUT_ACCESS_LEMMAS"
assert "NEXT_TARGET := extract expression-index scoped fold target-locality input access lemmas" in doc

print("UNGUARDED_FO_FINITE_BOOLEAN_FOLD_TARGET_FRAGMENT_INPUT_ACCESS_OK")

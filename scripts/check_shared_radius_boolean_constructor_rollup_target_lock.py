...
ART = Path("artifacts/cslib_fmt/shared_radius_boolean_constructor_rollup_target_lock_2026_06_20.json")
DOC = Path("docs/status/SHARED_RADIUS_BOOLEAN_CONSTRUCTOR_ROLLUP_TARGET_LOCK_2026_06_20.md")
LEAN = Path("lean/CSLIB/FMT/UnguardedFO/LocalityInputSurface.lean")

EXPECTED_STATUS = "SHARED_RADIUS_BOOLEAN_CONSTRUCTOR_ROLLUP_TARGET_LOCK_ONLY"
...
def main() -> None:
    ...
    assert artifact["object"] == "SharedRadiusBooleanConstructorRollupTarget"
    assert artifact["lean_file"] == str(LEAN)

    structure_pos = lean.index("structure SharedRadiusBooleanConstructorRollupTarget")
    ...
    for marker in [
        "Does not prove a rollup constructor theorem.",
    ...
    print("SHARED_RADIUS_BOOLEAN_CONSTRUCTOR_ROLLUP_TARGET_LOCK_OK")
